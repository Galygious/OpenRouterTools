...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function populateCreatorDropdown() {
    ...
    const creators = [...new Set(modelData.map(model => {
      ...
      const providerMatch = model.name?.match(/^([^:]+):/);
      return providerMatch ? providerMatch[1].trim() : model.provider;
    }))].filter(Boolean);
  ...
  function handleCreatorSelection() {
    ...
    const creatorModels = modelData.filter(model => {
      const providerMatch = model.name?.match(/^([^:]+):/);
      const provider = providerMatch ? providerMatch[1].trim() : model.provider;
      return provider === selectedCreator;
    });
