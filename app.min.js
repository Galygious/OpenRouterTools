  ...
  try {
    ...
    if (result && Array.isArray(result.data)) {
      result.data.forEach((model) => {
        ...
        allModelRecords[model.id] = model;
        ...
        // Store the mapping from display name to model ID
        modelSlugs[displayName] = model.id;
...
document.getElementById('fetch-all-button').addEventListener('click', async () => {
  // Get all models from the records
  const allModels = Object.values(allModelRecords);
  if (allModels.length === 0) {
    // If models haven't been loaded yet, fetch them first
    const { allModelRecords: newRecords } = await fetchModels();
    fetchStatsForAllModels(Object.keys(newRecords));
  } else {
    fetchStatsForAllModels(Object.keys(allModelRecords));
  }
});
...
async function fetchStatsForSingleModel(modelName) {
  ...
  try {
    const model = allModelRecords[modelId];
    if (!model) return tableRows;

    const row = document.createElement('tr');
    const pricePerMillion = (model.pricing?.prompt || 0) * 1000000;
    const completionPricePerMillion = (model.pricing?.completion || 0) * 1000000;
    const avgLatency = model.latency?.avg_ms ? (model.latency.avg_ms / 1000).toFixed(3) : 'N/A';
    ...
    row.innerHTML = `
      ...
      <td>${model.context_length || 'N/A'}</td>
      <td>$${pricePerMillion.toFixed(2)}</td>
      <td>$${completionPricePerMillion.toFixed(2)}</td>
      <td>${avgLatency}</td>
      <td>${model.throughput || 'N/A'}</td>
    `;
    tableRows.push(row);
  } catch (error) {
