    ...
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        response.data.forEach(config => {
          const promptPrice = parseFloat(config.pricing?.prompt || 0);
          const completionPrice = parseFloat(config.pricing?.completion || 0);
          const optionText = `${config.provider_display_name} ($${promptPrice}/prompt, $${completionPrice}/completion)`;
