...
async function fetchModels() {
  ...
  try {
    ...
    for (const item of result.data) {
      ...
      // Create model object with endpoint structure for compatibility
      const model = {
        name: item.id,
        endpoint: {
          model: {
          ...
          variant: '',
          model_variant_permaslug: item.id
        },
      ...
      modelSlugs[model.endpoint.model.name] = model.endpoint.model.permaslug;
...
function getAllMatches(query, records, excludedModels) {
  ...
  return Object.entries(records)
    .filter(([_, model]) => {
      return !excludedModels.includes(model.id);
    })
    .map(([_, model]) => {
      const normalizedOption = model.id.toLowerCase().trim();
      ...
      const pricePerMillion = (model.pricing?.prompt || 0) * 1000000;
      const displayName = `${model.id} ($${pricePerMillion.toFixed(2)}/1M tok)`;
...
document.getElementById('fetch-all-button').addEventListener('click', async () => {
  ...
  if (Object.keys(modelRecords).length === 0) {
    ...
    const { allModelRecords: newRecords } = await fetchModels();
    models = newRecords;
  } else {
    models = modelRecords;
  }
  
  const allModelNames = Object.values(models)
    .map(model => model.endpoint.model.name);
