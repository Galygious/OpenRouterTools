...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleVersionSelection() {
    ...
    // Find all models that match the selected version
    const configurations = modelData.filter(model => {
      console.log('Checking model:', model);
      const nameWithoutCreator = model.name.replace(new RegExp(`^${selectedCreator}:\\s*`), '');
      console.log('Name without creator:', nameWithoutCreator);
      const baseModelName = nameWithoutCreator.split(/\s+(?:v\d|[(]|$)/)[0].trim();
      console.log('Base model name:', baseModelName, 'Selected model:', selectedModel);
      const versionMatch = nameWithoutCreator.match(/\s+v(\d+(\.\d+)*)|[(](.*?)[)]/);
      const version = versionMatch ? versionMatch[0].trim() : 'default';
      console.log('Version:', version, 'Selected version:', selectedVersion);
      
      // Get creator from model name
      const creatorMatch = model.name?.match(/^([^:]+):/);
      const modelCreator = creatorMatch ? creatorMatch[1].trim() : model.provider;
      console.log('Model creator:', modelCreator, 'Selected creator:', selectedCreator);
      
      const isMatch = modelCreator === selectedCreator &&
    ...
    if (configurations.length > 0) {
      ...
      configurations.forEach(config => {
        const promptPrice = config.pricing?.prompt || 0;
        const completionPrice = config.pricing?.completion || 0;
        const configName = config.id.split('/').pop();
        const optionText = `${configName} ($${promptPrice}/prompt, $${completionPrice}/completion)`;
