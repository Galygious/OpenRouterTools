...
class ProviderSelector {
  ...
  selectBestProvider(constraints = {}) {
    ...
    return ranked.find(provider => {
      const price = provider.pricing?.prompt || 0;
    ...
    sendButton.addEventListener('click', async () => {
      ...
      try {
        ...
        const response = await this.inferenceManager.generateCompletion(
          prompt,
          model,
          this.providerSelector.selectBestProvider()
        );
  ...
  try {
    ...
    if (result && Array.isArray(result.data)) {
      result.data.forEach((item) => {
        ...
        const originalID = item.slug;
        const processedIdParts = originalID
          .split("/")
          .map((part) => part.replace(/[^\w]/g, "").toLowerCase());

        let currentLevel = nestedModel;
        processedIdParts.forEach((key, index) => {
          if (!currentLevel[key]) {
            currentLevel[key] = index === processedIdParts.length - 1 ? 
              { ...item } : {};
          }
      ...
      Object.entries(nestedModel).forEach(([topKey, subModels]) => {
        Object.values(subModels).forEach((model) => {
          allModelRecords[model.name] = model;
          modelSlugs[model.endpoint.model.name.trim()] = 
            model.endpoint.model.permaslug.trim() + 
            "&variant=" + 
            model.endpoint.variant.trim();
        });
      });
    }
  } catch (error) {
...
function getAllMatches(query, records, excludedModels) {
  ...
  return Object.entries(records)
    .filter(([_, modelObj]) => {
      return !excludedModels.includes(modelObj.endpoint.model.name);
    })
    .map(record => {
      const normalizedOption = record[1].endpoint.model_variant_permaslug.toLowerCase().trim();
      ...
      return {
        name: record[1].endpoint.model.name,
...
document.getElementById('fetch-all-button').addEventListener('click', () => {
...
async function fetchStatsForSingleModel(modelName) {
  const tableRows = [];
  const modelSlug = modelSlugs[modelName];
  if (!modelSlug) return tableRows;

  const API_URL = `https://openrouter.ai/api/frontend/stats/endpoint?permaslug=${modelSlug}`;
  try {
    const response = await fetch(API_URL, { method: 'GET' });
    if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
    ...
    data.data.forEach(provider => {
      const providerName = provider.provider_name || 'N/A';
      const contextLength = provider.context_length || 'N/A';
      const maxCompletionTokens = provider.max_completion_tokens || 
                                 provider.context_length || 'N/A';
      const pricing = provider.pricing || {};
      const pricingPrompt = (pricing.prompt !== undefined) ?
        (parseFloat(pricing.prompt) * 10 ** 6).toFixed(2) : 'N/A';
      const pricingCompletion = (pricing.completion !== undefined) ?
        (parseFloat(pricing.completion) * 10 ** 6).toFixed(2) : 'N/A';
      ...
      const avgLatencySec = latestStat.avg_latency ?
        (latestStat.avg_latency / 1000).toFixed(3) : 'N/A';
      const totalCompletionTokens = latestStat.total_completion_tokens || 0;
      const totalGenerationTimeMs = latestStat.total_generation_time || 1;
      const throughput = (totalCompletionTokens / (totalGenerationTimeMs / 1000)).toFixed(2);

      const row = document.createElement('tr');
      row.innerHTML = `
        <td>${provider.model.permaslug}</td>
        <td>${providerName}</td>
        <td>${contextLength}</td>
        <td>${maxCompletionTokens}</td>
        <td>$${pricingPrompt}</td>
        <td>$${pricingCompletion}</td>
        <td>${avgLatencySec}</td>
        <td>${throughput}</td>
      `;
      tableRows.push(row);
