...
class UIManager {
  ...
  async updateProviderList(modelName) {
    ...
    try {
      // Extract the base permaslug without variant
      const baseModelName = modelName.split(' ')[0];
      const permaslug = modelSlugs[modelName] || baseModelName;
      
      const response = await fetch(
        `https://openrouter.ai/api/frontend/stats/endpoint?permaslug=${permaslug}`
      );
      
      if (!response.ok) {
        // If the first attempt fails, try without the variant
        const baseResponse = await fetch(
          `https://openrouter.ai/api/frontend/stats/endpoint?permaslug=${baseModelName}`
        );
        ...
        this.providerSelector.setProviders(baseData.data);
      } else {
...
async function fetchModels() {
  ...
  modelSlugs = {}; // Reset modelSlugs to ensure clean state
  ...
  try {
    ...
    if (!result || !Array.isArray(result.data)) {
      throw new Error('Invalid response format from saved file');
    }

    if (result.data.length === 0) {
      throw new Error('No models returned from saved file');
    }
    ...
    result.data.forEach((item) => {
      ...
      const model = {
        ...
        endpoint: {
          model: {
            name: item.slug,
            permaslug: item.permaslug || item.slug
          },
          variant: '',
          model_variant_permaslug: item.permaslug || item.slug,
          pricing: item.endpoint?.pricing,
          provider_display_name: item.endpoint?.provider_display_name,
          max_completion_tokens: item.endpoint?.max_completion_tokens,
          latency: item.endpoint?.latency,
          throughput: item.endpoint?.throughput
        },
        context_length: item.context_length,
        provider: item.endpoint?.provider_name
      };
      ...
      // Store the mapping
      modelSlugs[displayName] = model.name;
    ...
    if (Object.keys(allModelRecords).length === 0) {
      throw new Error('No valid models found in saved file');
    }

    return { nestedModel, allModelRecords };
  } catch (error) {
    console.error("Error loading models from saved file:", error);
    throw error;
  }
}
